...
###Planning database
Pluto data from NYC planning department is used to provide addresses to get the latest data from Zillow (source:http://www1.nyc.gov/site/planning/data-maps/open-data/dwn-pluto-mappluto.page). Addresses obtained are then 
...
a<-getURL("https://github.com/cunyauthor/FinalProject/zillowdata.csv")
a<-getURL("https://raw.githubusercontent.com/cunyauthor/FinalProject/master/zillowdata.csv")
urlfile<-"https://github.com/cunyauthor/FinalProject/master/zillowdata.csv"
a<-read.csv(getURL(urlfile), encoding="UTF-8", na.strings=c("","NA"), stringsAsFactors = F)
x <- getURL(urlfile)
y <- read.csv(a,header=TRUE,sep="",stringsAsFactors = FALSE)
y<-read.csv(text=a,stringsAsFactors = FALSE)
```
...
zillowdata$bedrooms<-as.factor(zillowdata$bedrooms)
zillowdata$bathrooms<-as.factor(zillowdata$bathrooms)
...
zillow_clean %>% count(nb) %>% arrange(n)
zillow_clean %>% count(bedrooms)
zillow_clean %>% count(bathrooms)
